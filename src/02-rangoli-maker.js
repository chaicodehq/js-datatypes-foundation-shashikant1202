...
export function repeatPattern(pattern, times) {
  // Your code here
  if(typeof pattern !=="string" || times<0 || Number.isInteger(times)===false){
    return ""
  }
...
export function extractRangoliCenter(design, start, end) {
  // Your code here
  if(typeof design!=="string"  || start==null ||end==null){
    return "";
  }
...
export function makeRangoliBorder(char, length) {
  // Your code here
  if(typeof char !== "string" || length<0 || Number.isInteger(length)===false){
    return "";
  }
