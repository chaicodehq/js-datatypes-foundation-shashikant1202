...
export function calculateTotal(items) {
  ...
  // let price,qty;
  let total = 0
  return items.reduce((total,items)=> total+(items.price*items.qty),0)
}
...
export function sortByPrice(items, ascending) {
  ...
  a=items.price;
  b=items.price;
  if(ascending === true) return [...items].sort((a,b)=>a.price-b.price);
  else if(ascending === false) return [...items].sort((a,b)=>b.price-a.price);
  else return [];
}
